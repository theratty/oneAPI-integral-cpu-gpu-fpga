...
std::shared_ptr<sycl::device_selector> buildSelector(std::string selectorName)
{
    ...
    else if (selectorName == "fpga_emu")
    {
        return std::make_shared<sycl::INTEL::fpga_emulator_selector>();
    }
    else
    {
...
int main()
{
    std::shared_ptr<sycl::device_selector> deviceSelector{buildSelector("fpga_emu")};
